...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'CLIP':   return { ...state, currentClipIndex: action.index, currentFrame: 0 };
    case 'TICK': {
...
export function DriveReplayPlayer({ driveId, gameId, clips = [], week, season }: DriveReplayPlayerProps) {
  const [state, dispatch] = useReducer(reducer, initialState);
  const bufferRef = useRef(new ReplayBuffer(1200));
  const socketRef = useRef<ReplaySocket | null>(null);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Derive defense/offense from clip data
  const firstClip = clips[0];
  const defenseTeam = firstClip?.defense_team_id ?? undefined;
  const offenseTeam = firstClip?.offense_team_id ?? undefined;
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        const res = await fetch(`/api/replay-session/drive/${encodeURIComponent(driveId)}`, {
          method: 'POST',
        });
  ...
  const handlePrevClip = useCallback(() => {
    const prevIdx = Math.max(0, state.currentClipIndex - 1);
    const clip = clips[prevIdx];
    if (clip) dispatch({ type: 'SEEK', frame: clip.frame_start });
    dispatch({ type: 'CLIP', index: prevIdx });
  }, [state.currentClipIndex, clips]);

  const handleNextClip = useCallback(() => {
    const nextIdx = Math.min(clips.length - 1, state.currentClipIndex + 1);
    const clip = clips[nextIdx];
    if (clip) dispatch({ type: 'SEEK', frame: clip.frame_start });
    dispatch({ type: 'CLIP', index: nextIdx });
  }, [state.currentClipIndex, clips]);
  ...
  return (
    <div className="flex flex-col">
        ...
        {hasRealData ? (
          ...
          >
            <div style={{ textAlign: 'center' }}>
              ...
              >
                {state.status === 'error' ? 'REPLAY UNAVAILABLE' : 'LOADING PLAY DATA'}
              </div>
      ...
      <ReplayControls
        ...
        onPrevClip={clips.length > 1 ? handlePrevClip : undefined}
        onNextClip={clips.length > 1 ? handleNextClip : undefined}
      />
    </div>
  );
}
